...
export const Button = ({ title, loading, variant = 'primary', iconName, style, onPress, disabled, ...props }: ButtonProps) => {
  ...
  return <TouchableOpacity {...props} accessibilityRole="button" accessibilityState={{ disabled: Boolean(loading || disabled), busy: Boolean(loading) }} style={[styles.base, palette, (loading || disabled) && styles.disabled, style]} disabled={loading || disabled} activeOpacity={0.82} onPress={handlePress}>
