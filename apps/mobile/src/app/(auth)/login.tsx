...
export default function LoginScreen() {
  ...
  return <ScreenContainer scroll contentContainerStyle={styles.scroll}>
    ...
    <Card style={styles.form}><Input label="Email address" autoCapitalize="none" autoComplete="email" keyboardType="email-address" value={email} onChangeText={setEmail} error={formError?.includes('email') ? formError : undefined} /><Input label="Password" autoComplete="current-password" textContentType="password" maxLength={128} secureTextEntry value={password} onChangeText={setPassword} error={formError?.includes('password') ? formError : undefined} />{error && <Text accessibilityRole="alert" style={[styles.error, { color: colors.danger }]}>{error}</Text>}<Button title="Sign in securely" iconName="arrow-right" loading={isLoading} onPress={handleSignIn} /></Card>
