...
export const FinalCTA = () => {
  ...
  return <section id="contact" className="relative w-full bg-[#03050A] py-36 border-t border-white/[0.02] overflow-hidden flex flex-col items-center">
    ...
    <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ delay: 0.3 }} className="flex flex-col sm:flex-row gap-4"><Link to={user ? ROUTES.dashboard.overview : ROUTES.auth.login} className="group flex items-center justify-center gap-2 h-14 px-10 text-base font-bold text-black bg-white rounded-full transition-transform hover:scale-105">{user ? 'Open Operations' : 'Sign in to Operations'}<ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></Link><Link to={`${ROUTES.auth.login}?mode=register`} className="flex items-center justify-center h-14 px-10 text-base font-bold text-white bg-white/5 border border-white/10 hover:bg-white/10 rounded-full transition-colors">Create a citizen account</Link></motion.div>
    </div>
  </section>;
};
