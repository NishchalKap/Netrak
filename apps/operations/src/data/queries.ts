...
export function useCases() {
  return useQuery({ queryKey: queryKeys.cases, queryFn: ({ signal }) => caseRepository.list(signal), staleTime: 15000, gcTime: 600000, refetchInterval: POLL_INTERVAL, networkMode: 'offlineFirst' });
}
...
export function useThreats() {
  return useQuery({ queryKey: queryKeys.threats, queryFn: ({ signal }) => threatRepository.list(signal), staleTime: 60000, gcTime: 900000, refetchInterval: POLL_INTERVAL, networkMode: 'offlineFirst' });
}

export function useNotifications() {
  return useQuery({ queryKey: queryKeys.notifications, queryFn: ({ signal }) => notificationRepository.list(signal), staleTime: 30000, gcTime: 600000, refetchInterval: POLL_INTERVAL, networkMode: 'offlineFirst' });
}

export function useHealth() {
  return useQuery({ queryKey: queryKeys.health, queryFn: ({ signal }) => systemRepository.health(signal), staleTime: 15000, refetchInterval: POLL_INTERVAL });
}
...
export function useCaseActions() {
  ...
  return { createCase, updateCase, addEvidence };
}
